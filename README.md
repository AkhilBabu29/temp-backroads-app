...
//As we have separated all images and kept in a images folder we are importing an image from that..

// JSX
;<img src={logo} className="nav-logo" alt="backroads" />
//Same as we write in JSX, using that imported name in props to access it..
```
...
<!-- In HTML we add   #   to define an id and in react we write   id = '' to define id -->
...
/* For smooth scrolling we have used scroll behavior */
.section {
  /* navbar height */
...
//Here this list pattern was written as hard coded data, so to use it dynamically we will create the recurring data as an object and we will iterate through them..Here href and text are repeating..So, we create a data.js file for to save this data to change hard coded data into dynamic data and we will create objects with this data and will iterate..
```
...
//Here we have created an array of objects in the data.js and we will iterate through this..
```
...
//Here we are using map function to map the objects and then we are dynamically using it..
```
...
```js
//Same thing we are doing here for the social links, we are removing the hard coded data and changing it to dynamic data
{
...
<!-- {newDate().getFullYear()} gives current year in the footer -->
